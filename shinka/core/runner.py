...
def _extract_first_evolve_block_payload(code: str) -> Optional[str]:
    """Extract the first EVOLVE-BLOCK payload (content between START/END markers)."""
    in_block = False
    payload_lines: List[str] = []
    for line in str(code).splitlines():
        if re.match(r"^\s*(?:#|//|--)?\s*EVOLVE-BLOCK-START\b", line):
            in_block = True
            continue
        if re.match(r"^\s*(?:#|//|--)?\s*EVOLVE-BLOCK-END\b", line):
            break
        if in_block:
            payload_lines.append(line)
    if not payload_lines and not in_block:
        return None
    return "\n".join(payload_lines).strip()


def _lean_local_diff_stats(parent_code: str, child_code: str) -> dict:
    """Lightweight, order-tolerant change stats for Lean diff=local-full (no gating)."""
    parent_payload = _extract_first_evolve_block_payload(parent_code)
    child_payload = _extract_first_evolve_block_payload(child_code)
    if parent_payload is None or child_payload is None:
        return {
            "ok": False,
            "reason": "missing_evolve_block",
            "parent_has_block": parent_payload is not None,
            "child_has_block": child_payload is not None,
        }

    def normalize(stmt: str) -> str:
        stmt = stmt.strip()
        stmt = re.sub(
            r"\b(theorem|lemma|def|example|abbrev|opaque|axiom|instance)\s+[A-Za-z0-9_']+",
            r"\1 _NAME_",
            stmt,
        )
    ...
    def tokenize(stmt: str) -> List[str]:
        # Simple tokenization: identifiers / numbers / single chars.
        return re.findall(r"[A-Za-z_][A-Za-z0-9_']*|[0-9]+|[^\s]", stmt)

    parent_norm = normalize(parent_payload)
    child_norm = normalize(child_payload)
    parent_tokens = tokenize(parent_norm)
    child_tokens = tokenize(child_norm)

    c1 = Counter(parent_tokens)
    c2 = Counter(child_tokens)
    inter = sum((c1 & c2).values())
    union = sum((c1 | c2).values())
    added = sum((c2 - c1).values())
    removed = sum((c1 - c2).values())

    denom = max(len(parent_tokens), 1)
    return {
        "ok": True,
        "parent_chars": len(parent_norm),
        "child_chars": len(child_norm),
        "parent_tokens": len(parent_tokens),
        "child_tokens": len(child_tokens),
        "token_len_ratio": float(len(child_tokens)) / float(denom),
        "token_jaccard_multiset": float(inter) / float(union) if union > 0 else 1.0,
        "token_added": int(added),
        "token_removed": int(removed),
        "sequence_ratio": difflib.SequenceMatcher(None, parent_norm, child_norm).ratio(),
    }
...
class EvolutionRunner:
    def __init__(
        ...
        if evo_config.novelty_llm_models is not None:
            novelty_base_url = os.environ.get("OPENAI_NOVELTY_LLM_BASE_URL")
            self.novelty_llm = LLMClient(
                model_names=evo_config.novelty_llm_models,
                base_url=novelty_base_url,
    ...
    def generate_initial_program(self):
        ...
        for attempt in range(self.evo_config.max_patch_attempts):
            ...
            if not initial_code and self.evo_config.language == "lean":
                ...
                match = keyword_pattern.search(cleaned)
                if match:
                    initial_code = cleaned[match.start():].strip()
    ...
    def _submit_new_job(self):
        ...
        if current_gen == 0:
            ...
            novelty_similarity_top5: List[float] = []
                ...
                if self.novelty_judge.should_check_novelty(
                    ...
                    try:
                        novelty_max_similarity = float(
                            novelty_metadata.get("max_similarity", 0.0) or 0.0
                        )
                        sims = list(novelty_metadata.get("similarity_scores", []) or [])
                        sims_sorted = sorted((float(s) for s in sims), reverse=True)
                        novelty_similarity_top5 = sims_sorted[:5]
                    except Exception:
                        novelty_max_similarity = 0.0
                        novelty_similarity_top5 = []
        ...
        if current_gen > 0 and novelty_checks_performed > 0:
            ...
            meta_patch_data["novelty_max_similarity"] = novelty_max_similarity
            meta_patch_data["novelty_similarity_top5"] = novelty_similarity_top5
    ...
    def run_patch(
        ...
        cross_inspiration_ids = getattr(
            self.prompt_sampler, "_last_cross_inspiration_ids", []
        )
        ...
        for patch_attempt in range(max_patch_attempts):
            ...
            if error_attempt is None and num_applied_attempt > 0:
                ...
                if (
                    ...
                    and updated_code_attempt is not None
                ):
                    diff_local_stats = _lean_local_diff_stats(
                        parent_program.code, updated_code_attempt
                    )
                if self.verbose:
                    logger.info(
        ...
        if evaluated_code != "":
            ...
            try:
                if self.embedding is not None:
                    ...
                    # Lean theorem statements have high naming noise; normalize the decl name
                    # before embedding so novelty filtering is not defeated by trivial renames.
                    if str(getattr(self.evo_config, "language", "")).strip().lower() == "lean":
                        normalized = os.environ.get(
                            "AUTOFORMAL_EMBED_NORMALIZED_DECL_NAME", "my_theorem"
                        ).strip() or "my_theorem"
                        # Normalize only the first top-level decl name token.
                        redacted_code = re.sub(
                            r"^\s*(theorem|lemma|def|example|abbrev|opaque|axiom|instance)\s+[A-Za-z0-9_']+",
