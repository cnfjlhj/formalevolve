...
def _ensure_evolve_marker_newlines(text: str) -> str:
    """Ensure EVOLVE-BLOCK markers stay on their own lines.

    Motivation (Lean deterministic bug):
    - Some fallback/full outputs may contain glued markers, e.g.
      `-- EVOLVE-BLOCK-STARTtheorem ...`
      which comments out the declaration and later causes "Empty statement".

    Policy:
    - If START is not followed by optional spaces + newline, insert a newline after it.
    - If END is not preceded by a newline, insert a newline before it.
    """
    if not text:
        return text
    ...
    out = EVOLVE_END.sub(_repl_end, out)
    return out


def _extract_lean_fallback(raw: str) -> Optional[str]:
    """
    Fallback extraction for Lean code when SEARCH/REPLACE format is not found.
...
def _mutable_ranges(text: str) -> list[tuple[int, int]]:
    ...
    markers.sort(key=lambda kv: kv[1].start())
    ...
    if num_applied == 0 and language == "lean":
        ...
        if fallback_code:
            ...
            if mutable_ranges:
                ...
                suffix = original[end:]
                replacement = fallback_code.strip("\n")
