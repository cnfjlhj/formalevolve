...
_LEAN_EVOLVE_MARKER_RE = re.compile(r"^\s*(?:#|//|--)?\s*EVOLVE-BLOCK-(?:START|END)\b")
_LEAN_DECL_KEYWORDS_FOR_DEDUP = (
    "theorem",
    "lemma",
    "example",
    "def",
...
_LEAN_DECL_START_RE = re.compile(
    rf"^\s*(?:noncomputable\s+)?(?:{'|'.join(_LEAN_DECL_KEYWORDS_FOR_DEDUP)})\b"
)


def _strip_lean_evolve_markers(text: str) -> str:
    if not text:
        return ""
    kept: list[str] = []
    for line in str(text).splitlines():
        if _LEAN_EVOLVE_MARKER_RE.match(line):
            continue
        kept.append(line)
    return "\n".join(kept)


def _normalize_lean_code_for_exact_match(text: str) -> str:
    """Best-effort normalization for exact-duplicate detection (no dependencies)."""
    if not text:
        return ""
    raw = str(text)
    ...
    if m:
        ...
        if m2:
            raw = m2.group(1)
    raw = _strip_lean_evolve_markers(raw)
    return raw.strip()


def _extract_lean_preamble_and_first_decl(code: str) -> tuple[str, str]:
    s = _normalize_lean_code_for_exact_match(code)
    if not s:
        return "", ""
    lines = s.splitlines()
    decl_idx: Optional[int] = None
    for i, ln in enumerate(lines):
        if _LEAN_DECL_START_RE.search(ln):
            decl_idx = i
            break
    if decl_idx is None:
        return "", ""

    # Preamble: keep only non-empty stripped lines.
    pre_lines = [ln.strip() for ln in lines[:decl_idx] if ln.strip()]
    preamble = "\n".join(pre_lines).strip()

    next_idx: Optional[int] = None
    for j in range(decl_idx + 1, len(lines)):
        if _LEAN_DECL_START_RE.search(lines[j]):
            next_idx = j
            break
    end = next_idx if next_idx is not None else len(lines)
    decl = "\n".join(lines[decl_idx:end]).strip()
    return preamble, decl


def _canonicalize_lean_for_exact_match(text: str) -> str:
    """Canonicalize Lean code for *exact duplicate* detection (no similarity scoring).

    This normalizes:
    - whitespace
    - the top-level declaration name token
    - lemma/theorem keyword (treat lemma as theorem)

    It keeps imports/opens/options as part of the signature (user requested full-file-level).
    """
    preamble, decl = _extract_lean_preamble_and_first_decl(text or "")
    if not decl:
        return ""

    # Drop leading attribute annotations like `@[simp]`.
    decl = re.sub(r"(?m)^\s*@\[.*\]\s*\n?", "", decl).strip()
    if not decl:
        return ""

    decl = re.sub(r"\s+", " ", decl).strip()
    decl = re.sub(r"^(noncomputable\s+)?lemma\b", r"\1theorem", decl)
    decl = re.sub(
...
class DatabaseConfig:
    ...
    # Cycle-consistency softmax parent selection parameters
    # Used when parent_selection_strategy == "cycle_softmax"
    cycle_softmax_temperature: float = 3.5
    # Parent usage penalty (diversity): downweight parents that have been used
    # frequently as a parent (proxied by children_count).
    # Set to 0.0 to disable.
    parent_usage_penalty_alpha: float = 0.05
...
class ProgramDatabase:
    ...
    def __init__(
        ...
        self.embedding_model = (str(embedding_model).strip() if embedding_model else "")
        # Only create an embedding client when explicitly enabled.
        # This avoids requiring external embedding credentials for runs that do
        # not use embedding-based novelty features.
        self.embedding_client = (
            EmbeddingClient(model_name=self.embedding_model)
            if (not read_only and self.embedding_model)
            else None
        )
    ...
    def _update_archive(self, program: Program) -> None:
        ...
        # Exact-duplicate deduplication (Lean only): if an identical canonicalized
        # program already exists in the archive for this island, do not insert.
        if count > 0 and str(getattr(program, "language", "")).strip().lower() == "lean":
            try:
                cand_key = _canonicalize_lean_for_exact_match(program.code or "")
            except Exception:
                cand_key = ""
            if cand_key:
                ...
                if island_idx is not None:
                    self.cursor.execute(
                        ...
                        WHERE p.correct = 1 AND p.island_idx = ? AND p.id != ?
                        """,
                        (island_idx, program.id),
                    )
                else:
                    self.cursor.execute(
                        """
                        SELECT p.id, p.code
                        FROM archive a JOIN programs p ON a.program_id = p.id
                        WHERE p.correct = 1 AND p.id != ?
                        """,
                        (program.id,),
                    )
                for row in self.cursor.fetchall() or []:
                    try:
                        other_key = _canonicalize_lean_for_exact_match(row["code"] or "")
                    except Exception:
                        other_key = ""
                    if other_key and other_key == cand_key:
                        logger.info(
                            "Program %s skipped for archive (exact duplicate of %s).",
                            program.id,
                            row["id"],
                        )
                        return
    ...
    @db_retry()
    def delete_initial_seeds(self) -> int:
        """
        Delete all initial seed programs from the database.

        Initial seeds are identified by metadata.patch_name starting with "initial_program".
        This should be called once a non-seed program with compile_ok=1 enters the archive,
        to prevent evolution from regressing to trivial seeds.

        Returns:
            int: Number of programs deleted
        """
        if self.read_only:
            raise PermissionError("Cannot delete programs in read-only mode.")
        if not self.cursor or not self.conn:
            raise ConnectionError("DB not connected.")

        # Find all initial seed program IDs
        self.cursor.execute(
            "SELECT id FROM programs WHERE json_extract(metadata, '$.patch_name') LIKE 'initial_program%'"
        )
        seed_ids = [row["id"] for row in self.cursor.fetchall()]

        if not seed_ids:
            logger.info("No initial seeds found to delete.")
            return 0

        logger.info(f"Deleting {len(seed_ids)} initial seed(s): {seed_ids}")

        # Check if best_program is among the seeds
        best_needs_update = self.best_program_id in seed_ids

        # Delete from archive first (foreign key constraint)
        placeholders = ",".join("?" * len(seed_ids))
        self.cursor.execute(
            f"DELETE FROM archive WHERE program_id IN ({placeholders})",
            seed_ids
        )

        # Delete from programs table
        self.cursor.execute(
            f"DELETE FROM programs WHERE id IN ({placeholders})",
            seed_ids
        )

        self.conn.commit()

        # Update best_program if needed
        if best_needs_update:
            self.best_program_id = None
            # Find new best program
            self.cursor.execute(
                "SELECT id, combined_score FROM programs WHERE correct = 1 "
                "ORDER BY combined_score DESC LIMIT 1"
            )
            row = self.cursor.fetchone()
            if row:
                self.best_program_id = row["id"]
                self._update_metadata_in_db("best_program_id", self.best_program_id)
                logger.info(f"Updated best_program to {self.best_program_id}")

        logger.info(f"Deleted {len(seed_ids)} initial seed(s) from database.")
        return len(seed_ids)

    @db_retry()
    def delete_file_seed0(self) -> int:
        """
        Delete the initial seed program(s) that came from `--init_program` (file seed0).

        Rationale:
        - Some workflows provide a compile-safe placeholder as seed0 (e.g. `: True := by sorry`)
          to guarantee Gen0 bootstrapping does not crash when LLM-generated seeds fail to compile.
        - Once any non-file compile_ok=1 program is discovered, we must remove the file seed0 so it
          cannot dominate parent/inspiration sampling.

        Identification:
        - Programs with `metadata.init_seed_source == "file"` (this flag is copied across islands).
        ...
        self.cursor.execute(
            "SELECT id FROM programs WHERE json_extract(metadata, '$.init_seed_source') = 'file'"
        )
        seed_ids = [row["id"] for row in self.cursor.fetchall()]

        if not seed_ids:
            logger.info("No file seed0 programs found to delete.")
            return 0

        logger.info(f"Deleting {len(seed_ids)} file seed0 program(s): {seed_ids}")
        ...
        logger.info(f"Deleted {len(seed_ids)} file seed0 program(s) from database.")
        return len(seed_ids)
