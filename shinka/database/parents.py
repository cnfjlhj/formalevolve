...
class CycleSoftmaxSamplingStrategy(ParentSamplingStrategy):
    """
    Cycle-consistency softmax parent selection.

    Interprets each eligible parent's stored `cycle_normalized_log_prob` (in metadata)
    as a log-likelihood signal, and samples parents proportionally to:
        exp(cycle_normalized_log_prob / T)
    with normalization performed at sampling time over the current eligible set.

    Notes:
    - Only considers `correct=1` programs (for autoformalization, this is equivalent
      to compile_ok=1).
    - Falls back to uniform sampling if the cycle score metadata is missing.
    """
    ...
    def sample_parent(self) -> Any:
        ...
        if not archive_rows:
            logger.warning("No archived programs found for cycle-softmax sampling.")
            if self.best_program_id:
                best_prog = self.get_program(self.best_program_id)
                if best_prog and (
                    self.island_idx is None or best_prog.island_idx == self.island_idx
        ...
        temperature = float(getattr(self.config, "cycle_softmax_temperature", 3.5))
        ...
        for row in archive_rows:
            ...
            prog = SimpleProgram(p_dict)
            lp = prog.metadata.get("cycle_normalized_log_prob", None)
            if lp is None:
                continue
            try:
                lp_f = float(lp)
            ...
            alpha = max(alpha, 0.0)
            try:
                children_count = int(getattr(prog, "children_count", 0) or 0)
            except Exception:
                children_count = 0
            penalty = (
                math.log(1.0 + alpha * float(max(children_count, 0)))
                if alpha > 0.0 and children_count > 0
                else 0.0
            )
            logits.append((lp_f / temperature) - penalty)
        ...
        if z <= 0 or not np.isfinite(z):
            probabilities = np.ones(len(eligible_programs)) / len(eligible_programs)
        else:
            probabilities = (exp_scores / z).tolist()

        selected_parent = np.random.choice(eligible_programs, p=probabilities)
        logger.info(
            f"Cycle-softmax sampled parent {selected_parent.id} "
            f"(Gen: {getattr(selected_parent, 'generation', 'NA')}, "
            f"p={max(probabilities):.4f}, "
            f"Island: {getattr(selected_parent, 'island_idx', None)})"
        )
        return self.get_program(selected_parent.id)
...
class CombinedParentSelector:
    ...
    def sample_parent(self, island_idx: Optional[int] = None) -> Any:
        ...
        elif strategy_name == "cycle_softmax":
            strategy = CycleSoftmaxSamplingStrategy(
                self.cursor,
                self.conn,
                self.config,
                self.get_program,
